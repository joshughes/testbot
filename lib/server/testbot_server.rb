...
class TestBotServer < Sinatra::Base
  set :run, true

  def self.valid_version?(runner_version)
      return Testbot.version == runner_version
  end

  if ENV['INTEGRATION_TEST']
    set :port, 22880
  else
    set :port, 2288 #Testbot::SERVER_PORT
  end
  ...
  get '/runners/ping' do
    return unless TestBotServer.valid_version?(params[:version])
